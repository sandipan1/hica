...
class FileManipulationAgent(Agent):
...
class FileManipulationTool:
    ...
    async def run_filemanipulation_task(self, task_description: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            sub_agent = FileManipulationAgent(tool_registry=sub_agent_registry)
            ...
            self.main_agent_thread.add_event(
                type="tool_call",
                data={
                    "intent": "run_filemanipulation_task",
...
def get_filemanipulation_tool(
    thread: Thread, memory: ConversationMemoryStore
) -> FileManipulationTool:
    """Factory function to create a FileManipulationTool instance."""
    return FileManipulationTool(thread, memory)
